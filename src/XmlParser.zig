...
pub const Callbacks = struct {
    ...
    charData: *const fn (ctx: ?*anyopaque, data: []const u8) anyerror!void,
};
...
pub fn init(alloc: Allocator, callbacks: Callbacks) Error!XmlParser {
    ...
    c.XML_SetCharacterDataHandler(parser, charData);
...
fn charData(ctx: ?*anyopaque, data: [*c]const u8, len: c_int) callconv(.C) void {
    const inner: *Inner = @ptrCast(@alignCast(ctx));

    inner.callbacks.charData(inner.callbacks.ctx, data[0..@intCast(len)]) catch |e| {
        _ = c.XML_StopParser(inner.parser, 0);
        inner.err = e;
    };
}
