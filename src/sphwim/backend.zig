...
pub const Drm = struct {
    ...
    pub fn displayDmaBuf(self: *Drm,
        ...
        strides[0] = stride;
        offsets[0] = offset;
        modifierss[0] = modifiers;
        if (c.drmModeAddFB2WithModifiers(self.dri_file.handle, @intCast(width), @intCast(height), format, &handles, &strides, &offsets, &modifierss, &fb_id, c.DRM_MODE_FB_MODIFIERS) != 0) {
            const errno = std.c._errno();
            std.debug.print("err code: {d}\n", .{errno.*});

            if (c.drmModeAddFB2(self.dri_file.handle, @intCast(width), @intCast(height), format, &handles, &strides, &offsets, &fb_id, 0) != 0) {
                std.debug.print("err code: {d}\n", .{errno.*});
                return error.AddFb;
            }
        }
